...
from snips_nlu_ontology import get_all_languages, \
    get_supported_grammar_entities
...
def get_builtin_entity_scope(dataset, intent=None):
    language = dataset[LANGUAGE]
...
def get_dataset_builtin_entities(dataset, intent=None):
    if intent is not None:
        return extract_intent_entities(dataset, is_builtin_entity)[intent]
    return {e for e in dataset[ENTITIES] if is_builtin_entity(e)}
...
def get_dataset_grammar_entities(dataset, intent=None):
    if intent is not None:
        return extract_intent_entities(dataset, is_grammar_entity)[intent]
    return {e for e in dataset[ENTITIES] if is_grammar_entity(e)}
